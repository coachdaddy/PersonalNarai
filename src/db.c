...
read_mobile (int nr, int type)
{
  ...
  if (tmp < 2)
    ...
    {				/* 1 < tmp <= 20 */
      mob->points.hit = mob->points.max_hit =
	number (mob_bal_hit[level - 1][tmp - 2], mob_bal_hit[level - 1][tmp
									- 1]);
    }
  ...
  if (tmp < 2)
    ...
    {				/* 1 < tmp <= 20 */
      mob->points.exp =
	number (mob_bal_exp[level - 1][tmp - 2], mob_bal_exp[level - 1][tmp
									- 1]);
    }
...
void
store_to_char_for_transform (struct char_file_u *st, struct char_data *ch)
{
  int i;
...
void
save_char_nocon (struct char_data *ch, sh_int load_room)
{
  FILE *fl;
  int player_i;
...
void
delete_char (struct char_data *ch)
{
...
int
remove_entry (struct char_data *ch)
{
  int i;
  int need_copy;
...
int
compare (struct player_index_element *arg1, struct player_index_element
	 *arg2)
{
...
void
move_stashfile (char *victim)	/* move file.x to file.x.y */
{
...
void
stash_char (struct char_data *ch)
{
  struct obj_data *p;
...
void
stash_contentsII (FILE * fp, struct obj_data *o, int wear_flag)
{
  struct obj_data *oc;
  int i;
...
/* old version */
void
stash_contents (FILE * fl, struct obj_data *p, int wear_flag)
{
  struct obj_data *pc;
  int j;
...
/* old version */
void
unstash_char (struct char_data *ch, char *filename)
{
...
void
stash_contents (FILE * fp, struct obj_data *o, int wear_flag)
{
  struct obj_data *oc;
  int i;
...
void
wipe_stash (char *filename)	/* delete id.x and id.x.y */
{
...
void
do_checkrent (struct char_data *ch, char *argument, int cmd)
{
...
void
do_extractrent (struct char_data *ch, char *argument, int cmd)
{
...
void
do_replacerent (struct char_data *ch, char *argument, int cmd)
{
  char name[MAX_INPUT_LENGTH], buf[MAX_OUTPUT_LENGTH];
...
void
do_rent (struct char_data *ch, int cmd, char *arg)
{
  // sh_int save_room;
  int i;
