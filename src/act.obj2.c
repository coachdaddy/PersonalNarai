...
void
do_drink (struct char_data *ch, char *argument, int cmd)
{
  char buf[100];
	  ...
	  switch (temp->obj_flags.value[3])
	    ...
	    case 2:		/* hit point restore */
	      ...
	      GET_HIT (ch) = MIN (GET_MAX_HIT (ch), GET_HIT (ch) +
				  dice
				  (drink_aff[temp->obj_flags.value[2]][DRUNK], 8));
	      break;
	    case 3:		/* mana restore */
	      act ("You feel momentarily warm.", FALSE, ch, 0, 0, TO_CHAR);
	      ...
	      GET_MANA (ch) = MIN (GET_MAX_MANA (ch), GET_MANA (ch) +
				   dice
				   (drink_aff[temp->obj_flags.value[2]][DRUNK], 7));
	      break;
	    case 4:		/* move restore */
	      break;
	    case 0:		/* nothing */
...
void
do_eat (struct char_data *ch, char *argument, int cmd)
{
  char buf[100];
  struct obj_data *temp;
  struct affected_type af;
...
void
do_pour (struct char_data *ch, char *argument, int cmd)
{
...
void
do_sip (struct char_data *ch, char *argument, int cmd)
{
  struct affected_type af;
  ...
  gain_condition (ch, DRUNK, (int)
		  (drink_aff[temp->obj_flags.value[2]][DRUNK] / 4));

  gain_condition (ch, FULL, (int) (drink_aff[temp->obj_flags.value[2]][FULL]
				   / 4));

  gain_condition (ch, THIRST, (int)
		  (drink_aff[temp->obj_flags.value[2]][THIRST] / 4));
...
void
do_taste (struct char_data *ch, char *argument, int cmd)
{
  struct affected_type af;
  char arg[MAX_STRING_LENGTH];
  struct obj_data *temp;
...
void
wear (struct char_data *ch, struct obj_data *obj_object, int keyword)
{
...
int
where_wear (struct obj_data *obj_object)
{
  if (CAN_WEAR (obj_object, ITEM_WEAR_KNEE))
    return (15);
  if (CAN_WEAR (obj_object, ITEM_WEAR_ABOUTLEGS))
...
void
do_wear (struct char_data *ch, char *argument, int cmd)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char buf[MAX_OUTPUT_LENGTH];
...
void
do_wield (struct char_data *ch, char *argument, int cmd)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_grab (struct char_data *ch, char *argument, int cmd)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char buffer[MAX_OUTPUT_LENGTH];
...
void
do_unwield (struct char_data *ch, char *argument, int cmd)
{
  struct obj_data *obj_object;

  obj_object = ch->equipment[WIELD];
  if (obj_object)
    {
    ...
    {
      send_to_char ("You are not wielding it.\n\r", ch);
    }
}

void
do_unhold (struct char_data *ch, char *argument, int cmd)
{
  struct obj_data *obj_object;
  ...
  if (obj_object)
    {
      if (CAN_CARRY_N (ch) != IS_CARRYING_N (ch))
	{
	  act ("$n stops using $p.", FALSE, ch, obj_object, 0, TO_CHAR);
	  act ("$n stops using $p.", TRUE, ch, obj_object, 0, TO_ROOM);
    ...
    {
      send_to_char ("You are not holding it.\n\r", ch);
    }
}

void
do_remove (struct char_data *ch, char *argument, int cmd)
{
	  ...
	  for (i = 0; i < MAX_WEAR; i++)
	    {
	      ...
	      if (obj_object && CAN_CARRY_N (ch) != IS_CARRYING_N (ch))
		{
