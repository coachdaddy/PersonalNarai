	...
	if (temp->obj_flags.type_flag == ITEM_DRINKCON) {
		if (temp->obj_flags.value[1] > 0)	/* Not empty */
		{
			...
			if (GET_COND(ch, THIRST) > 32)
				act("You do not feel thirsty.", FALSE, ch, 0,
				    0, TO_CHAR);
			...
			case 1:	/* The shit was poisoned ! */
				act("Ooups, it tasted rather strange ?!!?",
			...
			case 2:	/* hit point restore */
				act("You feel momentarily warm.", FALSE, ch,
				    0, 0, TO_CHAR);
				acthan("$n blushes.",
				       "$n님의 얼굴이 붉어집니다.",
				       TRUE, ch, 0, 0, TO_ROOM);
...
void do_eat(struct char_data *ch, char *argument, int cmd)
{
	...
	if ((temp->obj_flags.value[3] == 1) && (GET_LEVEL(ch) < IMO)) {
		...
		act("$n coughs and utters some strange sounds.", FALSE, ch, 0,
		    0, TO_ROOM);
		af.type = SPELL_POISON;
		  ...
		  int keyword)
{
	...
	case 13:
		...
		break;
	case 14:
		act("$n starts using $p as a shield.", TRUE, ch, obj_object,
		    0, TO_ROOM);
		break;
	case 15:
...
void do_remove(struct char_data *ch, char *argument, int cmd)
{
	...
	if (*arg1) {
		if (strcmp(arg1, "all")) {
			obj_object = get_object_in_equip_vis(ch, arg1,
							     ch->equipment, &j);
			if (obj_object) {
				if (CAN_CARRY_N(ch) != IS_CARRYING_N(ch)) {
