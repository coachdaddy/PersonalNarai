...
void do_steal(struct char_data *ch, char *argument, int cmd)
{
	...
	if (str_cmp(obj_name, "coins") && str_cmp(obj_name, "gold")) {
		if (!(obj = get_obj_in_list_vis(victim, obj_name, victim->carrying))) {
			...
			if (AWAKE(victim) && (percent >
					      ch->skills[SKILL_STEAL].learned)) {
				ohoh = TRUE;
				act("Oops..", FALSE, ch, 0, 0, TO_CHAR);
				act("$n tries to rob $N.", TRUE, ch, 0,
				    victim, TO_NOTVICT);
				if (!IS_NPC(victim))
					act("$n tried to rob you.", TRUE, ch,
					    0, victim, TO_VICT);
			} else {	/* Steal the item */
		...
		if (AWAKE(victim) && (percent > ch->skills[SKILL_STEAL].learned)) {
			ohoh = TRUE;
			act("Oops..", FALSE, ch, 0, 0, TO_CHAR);
			act("$n tries to steal gold from $N.", TRUE, ch, 0,
			    victim, TO_NOTVICT);
			if (!IS_NPC(victim))
				act("$n tried to rob you.", TRUE, ch, 0,
				    victim, TO_VICT);
		} else {
...
void do_practice(struct char_data *ch, char *arg, int cmd)
{
	...
	for (i = 0; *spells[i] != '\n'; i++) {
		...
		if (*spells[i + 1] != '\n') {
			i++;
			if (!(spell_info[i + 1].min_level[GET_CLASS(victim) -
			      1] >
			      GET_LEVEL(victim)) &&
...
void do_group(struct char_data *ch, char *argument, int cmd)
{
		...
		if (ch && !IS_AFFECTED(ch, AFF_GROUP)) {
			...
			if (k && IS_AFFECTED(k, AFF_GROUP)) {
				snprintf(buf, sizeof(buf),
					...
					GET_MANA(k),
					GET_PLAYER_MAX_MANA(k), GET_MOVE(k),
					GET_PLAYER_MAX_MOVE(k));
			...
			for (f = k->followers; f; f = f->next)
				if (f->follower && IS_AFFECTED(f->follower, AFF_GROUP)) {
					snprintf(buf, sizeof(buf),
						...
						GET_MANA(f->follower),
						GET_PLAYER_MAX_MANA(f->follower),
						GET_MOVE(f->follower),
						GET_PLAYER_MAX_MOVE(f->follower));
	...
	if (!(victim = get_char_room_vis(ch, name))) {
		...
		if (IS_NPC(victim)) {
			act("You can not enroll mob into your group.", FALSE,
			    ch, 0, 0, TO_CHAR);
		...
		if (found) {
			if (victim && IS_AFFECTED(victim, AFF_GROUP)) {
				act("$n has been kicked out of the group!",
				    FALSE, victim, 0,
				    ch, TO_ROOM);
				act("You are no longer a member of the group!", FALSE,
				    victim, 0, 0, TO_CHAR);
			...
			act("$N must follow you, to enter the group", FALSE,
			    ch, 0,
			    victim, TO_CHAR);
		}
	}
...
void do_disarm(struct char_data *ch, char *argument, int cmd)
{
	...
	if (percent < ch->skills[SKILL_DISARM].learned) {
		...
		if (obj && CAN_SEE_OBJ(ch, obj)) {
			...
			act("$E is not wielding any weapon now.", FALSE, ch,
			    0, victim, TO_CHAR);
			return;
		}
	} else {
...
void do_throw_object(struct char_data *ch, char *argument, int cmd)
{
	...
	if (obj) {
		if (!IS_SET(obj->obj_flags.extra_flags, ITEM_NODROP)) {
			...
			dam += (GET_LEVEL(ch) + GET_STR(ch) + GET_ADD(ch) +
				GET_CON(ch));
			dam *= ((GET_LEVEL(ch) + GET_SKILLED(ch,
				SKILL_THROW_OBJECT)) >> 3);
			percent = number(1, 100) - GET_LEVEL(ch) + 2 *
			    GET_LEVEL(victim)
