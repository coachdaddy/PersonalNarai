      ...
      else
	{
	  ...
	  for (f = k->followers; f; f = f->next)
	    if (f->follower && IS_AFFECTED (f->follower, AFF_GROUP))
	      {
		sprintf (buf, "  [ %5ld/%5ld %5ld/%5ld %5ld/%5ld ]   $N",
  ...
  if (!(victim = get_char_room_vis (ch, name)))
      ...
      if (found)
	{
	  if (victim && IS_AFFECTED (victim, AFF_GROUP))
	    {
	      act ("$n has been kicked out of the group!", FALSE, victim, 0,
		   ch, TO_ROOM);
	...
	{
	  act ("$N must follow you, to enter the group", FALSE, ch, 0,
	       victim, TO_CHAR);
	}
    }
}
...
void
do_post (struct char_data *ch, char *argument, int cmd)
{
...
*/
void
do_assault (struct char_data *ch, char *argument, int cmd)
{
  struct char_data *victim;
  struct obj_data *obj;
...
void
do_spin_bird_kick (struct char_data *ch, char *argument, int cmd)
{
  unsigned int dam;
...
void
do_shouryuken (struct char_data *ch, char *argument, int cmd)
{
  unsigned int dam;
  struct char_data *victim;
  char victim_name[240];
...
void
do_throw_object (struct char_data *ch, char *argument, int cmd)
{
  struct char_data *victim;
  struct obj_data *obj;
  char victim_name[240];
