...
int
is_ok (struct char_data *keeper, struct char_data *ch, int shop_nr)
{
...
int
shop_keeper (struct char_data *ch, int cmd, char *arg)
{
  ...
  if (((cmd == 84) || (cmd == 207) || (cmd == 172)) && GET_LEVEL (ch) < IMO)
    {				/* Cast, recite, use */
