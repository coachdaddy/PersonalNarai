...
void do_power_bash(struct char_data *ch, char *argument, int cmd)
{
	...
	if (percent < GET_GUILD_SKILL(ch, POLICE_SKILL_POWER_BASH)) {
		...
		damage(ch, victim, GET_GUILD_SKILL(ch,
		       POLICE_SKILL_POWER_BASH), SKILL_BASH);
		if (percent < GET_GUILD_SKILL(ch, POLICE_SKILL_POWER_BASH))
			GET_POS(victim) = POSITION_STUNNED;
		if (victim)
			WAIT_STATE(victim,
				   (GET_GUILD_SKILL(ch,
				    POLICE_SKILL_POWER_BASH) / 20)
				   * PULSE_VIOLENCE);
	} else {
		GET_MOVE(ch) -= 1000;
		if (ch)
...
void do_simultaneous(struct char_data *ch, char *argument, int cmd)
{
	...
	for (vict = world[ch->in_room].people; vict; vict = tmp_victim) {
		...
		if ((vict &&
			...
			if (victim)
				act("$n found $N's a weak point!!", TRUE,
				    vict, 0, victim, TO_ROOM);
...
#define JALE_ROOM	1800
	...
	if (((GET_GUILD_SKILL(ch, POLICE_SKILL_ARREST) > number(1, 99) &&
		...
		act("$n disappears suddenly to JALE!!!!!!", TRUE, victim, 0,
		    0, TO_ROOM);
