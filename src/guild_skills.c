...
/* ************************************************************************
*  file: guild_skills.c , Guild skills module           				*
*  Usage: guild skills implementation									*
...
/* skills of police */
void
do_power_bash (struct char_data *ch, char *argument, int cmd)
{
  char name[256];
...
void
do_whistle (struct char_data *ch, char *argument, int cmd)
{
  char name[256];
  char buf[256];
  struct char_data *victim;
...
void
do_simultaneous (struct char_data *ch, char *argument, int cmd)
{
  char name[256];
...
void
do_arrest (struct char_data *ch, char *argument, int cmd)
{
  char name[256];
  struct char_data *victim;
...
#define JALE_ROOM	1800
  ...
  if (((GET_GUILD_SKILL (ch, POLICE_SKILL_ARREST) > number (1, 99) &&
	GET_HIT (victim) < GET_PLAYER_MAX_HIT (victim) / 8) && GET_MANA (ch)
       > 500) ||
      GET_LEVEL (ch) > (IMO - 1))
    {
...
/* skills of outlaws */
void
do_charge (struct char_data *ch, char *argument, int cmd)
{
  char name[256];
  struct char_data *victim;
  int dam;
...
/* just damage */
void
do_angry_yell (struct char_data *ch, char *argument, int cmd)
{
  char name[256];
  struct char_data *victim;
  int dam;
...
void
do_smoke (struct char_data *ch, char *argument, int cmd)
{
}

void
do_inject (struct char_data *ch, char *argument, int cmd)
{
}

/* skills of assasins */
void
do_shadow (struct char_data *ch, char *argument, int cmd)
{
  struct affected_type af;
...
void
do_solace (struct char_data *ch, char *argument, int cmd)
{
...
void
do_evil_strike (struct char_data *ch, char *argument, int cmd)
{
  int dam, percent;
  char name[256];
  struct char_data *victim;
