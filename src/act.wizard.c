	...
	if (!*arg1) {
		...
		if (k) {
			...
			if (IS_NPC(k)) {
				...
				strcpy(buf, "Title: ");
				strcat(buf, (k->player.title ? k->player.title
					     : "None"));
				strcat(buf, "\n\r");
				strcat(page_buffer, buf);
			}
			...
			if (IS_NPC(k)) {
				strcpy(buf, "Special: ");
				strcat(buf, (mob_index[k->nr].func ?
					     "Exists\n\r" :
					     "None\n\r"));
				strcat(page_buffer, buf);
			}
...
void do_set(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*buf) {
		snprintf(mess, sizeof(mess),
			...
			nodisarmflag, regen_percent, regen_time_percent,
			regen_time, reboot_time);	/* chase added nodisarm */
		...
		if (!(victim = get_char(buf))) {
			...
			if ((GET_LEVEL(ch) < (IMO + 2)) && (strcmp(buf3,
								   "gold")))
				return;
			if (strcmp(buf3, "exp") == 0 && (GET_LEVEL(ch) > (IMO
									  + 2)))
				...
				if (strncmp("m", buf4, 1) == 0 || strncmp("M",
									  buf4,
				    1) == 0)
					victim->player.sex = 1;
				else if (strncmp("f", buf4, 1) == 0 ||
									       strncmp("F",
									       buf4, 1)
					 == 0)
					victim->player.sex = 2;
