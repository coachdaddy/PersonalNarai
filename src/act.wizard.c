...
void
do_transform (struct char_data *ch, char *argument, int cmd)
{
...
void
do_wiznet (struct char_data *ch, char *argument, int cmd)
{
  struct descriptor_data *i;
  struct char_data *victim;
...
FILE *chatlogfp = NULL;
void
do_chat (struct char_data *ch, char *argument, int cmd)
{
  struct descriptor_data *i;
  struct char_data *victim;
  char buf[MAX_STRING_LENGTH];
...
void
do_noaffect (struct char_data *ch, char *argument, int cmd)
{
  struct char_data *vict;
  struct obj_data *dummy;
...
void
do_wall (struct char_data *ch, char *argument, int cmd)
{
  char buf[MAX_STRING_LENGTH];
  ...
  if (!*buf)
      ...
      else
	{
	  ...
	  if (strcmp (buf3, "exp") == 0 && (GET_LEVEL (ch) > (IMO + 2)))
		...
		victim->player.sex = 1;
	      else if (strncmp ("f", buf4, 1) == 0 || strncmp ("F", buf4, 1)
		       == 0)
		victim->player.sex = 2;
	      else
...
void
do_flick (struct char_data *ch, char *argument, int cmd)
{
  struct char_data *victim;
  struct obj_data *obj;
...
void
do_sys (struct char_data *ch, char *argument, int cmd)
{
  struct rusage xru;
  char buffer[256];
  extern int boottime;
