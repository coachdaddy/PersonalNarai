...
void log(char *str);
void load_zones(int zon);							/* in db.c, zreload 구현용, 251120 by Komo */
void reload_world_file(FILE *fl, int zone_rnum);	/* in db.c, wreload 구현용, 251121 by Komo */
...
/* send_to_room_except 대체 및 출력 방식 등 수정, 251126 by Komo */
...
static char *load_one_argument(char *argument, char *first_arg)
{
    ...
    while (*argument && !isspace(*argument)) {
        *first_arg = *argument; // 내용 복사
        first_arg++;            // 도착지 포인터 이동
        argument++;             // 원본 포인터 이동
    }
...
/* modified by Komo, 251115 */
void do_purge(struct char_data *ch, char *argument, int cmd)
{
	struct char_data *vict, *next_v;
	struct obj_data *obj, *next_o;
	int i;
	char name[100], buf[100], confirm[100]; // confirm 버퍼 추가
...
void roll_abilities(struct char_data *ch)
{
    ...
    // 능력치 5개 생성
    for (i = 0; i < 5; i++) {
        /* 1~7 사이의 숫자 4개를 더하는 원래의 로직을 dice 함수로 수정(4D7).
		 평균이 16이므로, 18이 나오기 매우 쉬운 구조, 251124 by Komo */
        result = dice(4, 7);
        
        if (result > 18) result = 18; /* 최대값은 18 */
	...
	for (i = descriptor_list; i; i = i->next)
		if (!i->connected) {
			if (i->original)
				continue;
			victim = i->character;
			if ((GET_LEVEL(victim) >= IMO) && (GET_LEVEL(victim)
							   <= (IMO + 4)))
...
void do_set(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*buf) {
        ...
        snprintf(mess, sizeof(mess),
				 ...
				 *baddomain[0] ? baddomain[0] : "<NONE>"); /* NULL 포인터 방지 */
		...
		/* char */
			...
			if (strcmp(buf3, "exp") == 0 && (GET_LEVEL(ch) > (IMO + 2)))
                ...
                strncpy(victim->desc->pwd, buf4, sizeof(victim->desc->pwd) - 1); /* 251121 by Komo */
...
/* ==========================================================================
*  Live Reloading 준비, 251120 by Komo
* ========================================================================= */
...
void do_zreload(struct char_data *ch, char *argument, int cmd)
{
    ...
    // 존 테이블에서 해당 '존 번호(number)'를 가진 인덱스(RNUM) 찾기
    for (i = 0; i <= top_of_zone_table; i++) {
        if (zone_table[i].number == target_zone_num) {
            zone_rnum = i; // yes, found.
            break;
        }
    }
...
void do_zonelist(struct char_data *ch, char *argument, int cmd)
{
    ...
    size_t buf_size = 65536; /* 64KB - 충분히 크다 */
