...
void
make_corpse (struct char_data *ch, int level)
{
...
void
raw_kill (struct char_data *ch, int level)
{
  if (ch->specials.fighting)
    stop_fighting (ch);
...
void
die (struct char_data *ch, int level, struct char_data *who)
{
...
char *
replace_string (char *str, char *weapon)
{
      ...
      if (IS_NPC (victim))
	{
	  if (IS_AFFECTED (ch, AFF_RERAISE))
	      ...
	      else
		{
		  ...
		  GET_GOLD (ch) += (GET_LEVEL (victim) * GET_LEVEL (victim)
				    * 500);
...
hit (struct char_data *ch, struct char_data *victim, int type)
{
  ...
  if (GET_LEARNED (victim, SKILL_PARRY))
      ...
      if (GET_CLASS (victim) == CLASS_THIEF)
	{
      ...
      if (number (0, parry_num) > (GET_HITROLL (ch) << 1) + GET_LEVEL (ch) -
	  GET_LEVEL (victim) + 43)
	{
	  prf = 1;
	}
      ...
      if (AWAKE (ch) && (ch->in_room == ch->specials.fighting->in_room))
	{
	  ...
	  if (!IS_NPC (ch))
	      ...
	      if (ch->skills[SKILL_OCTA].learned > 0)
		{
		  if (GET_CLASS (ch) == CLASS_MAGIC_USER || GET_CLASS (ch)
		      == CLASS_CLERIC)
		    if (number (1, 40) > 10 + (GET_SKILLED (ch, SKILL_OCTA)
					       >> 3))
		      goto octa;
