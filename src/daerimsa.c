...
int
son_ogong_func (struct char_data *ch, int cmd, char *arg)
{
	...
	case 0:
	  if (!cmd)
	  ...
	  else if (cmd == 13)
	    {
		  ...
		  &&
		  obj_index[ch->equipment[WEAR_HEAD]->item_number].virtual
		  == GOLDEN_RIM)
		{
...
int
fourth_jangro_func (struct char_data *ch, int cmd, char *arg)
{
  long gold;
...
int
teleport_daerimsa_tower (struct char_data *ch, int cmd, char *arg)
{
  ...
  if (cmd == 207)
      ...
      if (!(scroll = get_obj_in_list_vis (ch, buf, ch->carrying)))
	{
      ...
      if (obj_index[scroll->item_number].virtual != DAERIMSA_SCROLL)
	return 0;
...
int
saint_water (struct char_data *ch, int cmd, char *arg)
{
  char buf[100];
  struct obj_data *obj;
