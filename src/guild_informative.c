...
void
do_cant (struct char_data *ch, char *argument, int cmd)
{
	...
	if (!i->connected)
	  {
	    if (i->original)
	      continue;
	    victim = i->character;
	    if (ch->player.guild == victim->player.guild && GET_LEVEL
		(victim) < IMO)
	      {
		j++;
		sprintf (buf, "<%2d> %s <%5ld,%5ld,%5ld> %s \n\r", GET_LEVEL (victim),
...
void
do_query (struct char_data *ch, char *argument, int cmd)
{
  struct char_data *victim;
