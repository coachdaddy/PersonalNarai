...
void
do_assist (struct char_data *ch, char *argument, int cmd)
{
	  ...
	  if (victim == ch)
	    {
	      ...
	      act ("$n tries to help $mself, and says YAHA!", FALSE, ch, 0,
		   victim, TO_ROOM);
	    }
	  else
	    {
...
void
stop_group (struct char_data *ch)
{
...
void
do_ungroup (struct char_data *ch, char *argument, int cmd)
{
  struct follow_type *j, *k;
...
void
do_version (struct char_data *ch, char *argument, int cmd)
{
  extern char fileid[];
...
void
do_resetzone (struct char_data *ch, char *argument, int cmd)
{
...
void
do_reloadzone (struct char_data *ch, char *argument, int cmd)
{
  char buf[255], buf2[255];
  int i;
  if (!ch)
