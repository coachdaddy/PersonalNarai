...
*/
{
    ...
    int obj_number;
	int dest_room_rnum; // 목적지 RNUM 저장용
    int sect_from, sect_to; // 섹터 타입 가져오기
    ...
    // 섹터가 이상하면 SECT_FIELD로 간주 (다운 방지)
    if (sect_from < 0 || sect_from >= 9) sect_from = SECT_FIELD;
    if (sect_to < 0   || sect_to >= 9)   sect_to = SECT_FIELD;

	need_movement = (movement_loss[sect_from] + movement_loss[sect_to]) / 2;
...
void do_open(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				act("$n opens the $F.", FALSE, ch, 0, EXIT(ch,
				    door)->keyword,
				    TO_ROOM);
			else
...
void do_close(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				act("$n closes the $F.", 0, ch, 0, EXIT(ch,
				    door)->keyword,
				    TO_ROOM);
			else
...
void do_lock(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				act("$n locks the $F.", 0, ch, 0, EXIT(ch,
				    door)->keyword,
				    TO_ROOM);
			else
...
void do_unlock(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				act("$n unlocks the $F.", 0, ch, 0, EXIT(ch,
				    door)->keyword,
				    TO_ROOM);
			else
	...
	if (!*type)
		...
		if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
			...
			if (EXIT(ch, door)->keyword)
				...
				act("$n picks the lock of the.", TRUE, ch, 0,
				    0, TO_ROOM);
			...
			if (EXIT(ch, door))
				if (EXIT(ch, door)->to_room != NOWHERE)
					if (!IS_SET(EXIT(ch, door)->exit_info,
					    EX_CLOSED) &&
					    IS_SET(world[EXIT(ch,
						   door)->to_room].room_flags,
						   INDOORS)) {
						do_move(ch, "", ++door);
						return;
					}
			...
			if (EXIT(ch, door))
				if (EXIT(ch, door)->to_room != NOWHERE)
					if (!IS_SET(EXIT(ch, door)->exit_info,
					    EX_CLOSED) &&
					    !IS_SET(world[EXIT(ch,
						    door)->to_room].room_flags, INDOORS)) {
						do_move(ch, "", ++door);
						return;
					}
...
void do_stand(struct char_data *ch, char *argument, int cmd)
{
	...
	case POSITION_RESTING:
		{
			act("You stop resting, and stand up.", FALSE, ch, 0,
			    0, TO_CHAR);
...
void do_sit(struct char_data *ch, char *argument, int cmd)
{
	...
	case POSITION_FIGHTING:
		{
			act("Sit down while fighting? are you MAD?", FALSE,
			    ch, 0, 0, TO_CHAR);
		}
		break;
	default:
		{
			act("You stop floating around, and sit down.", FALSE,
			    ch, 0, 0, TO_CHAR);
			act("$n stops floating around, and sits down.", TRUE,
			    ch, 0, 0, TO_ROOM);
			GET_POS(ch) = POSITION_SITTING;
		}
		break;
...
void do_rest(struct char_data *ch, char *argument, int cmd)
{
	...
	case POSITION_STANDING:
		{
			act("You sit down and rest your tired bones.", FALSE,
			    ch, 0, 0, TO_CHAR);
	...
	default:
		{
			act("You stop floating around, and stop to rest your tired bones.",
			    FALSE, ch, 0, 0, TO_CHAR);
			act("$n stops floating around, and rests.", FALSE, ch,
			    0, 0, TO_ROOM);
			GET_POS(ch) = POSITION_SITTING;
		}
		break;
