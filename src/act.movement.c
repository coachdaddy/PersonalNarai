...
int
do_simple_move (struct char_data *ch, int cmd, int following)
/* Assumes, 
...
void
do_move (struct char_data *ch, char *argument, int cmd)
{
  char tmp[80];
  int was_in;
		  ...
		  if (ch->followers)
		      ...
		      for (k = ch->followers; k; k = next_dude)
			{
			  ...
			  if ((was_in == k->follower->in_room) &&
			    ...
			    {
			      act ("You follow $N.", FALSE, k->follower, 0,
				   ch, TO_CHAR);
			      cmd++;
			      send_to_char ("\n\r", k->follower);
...
int
find_door (struct char_data *ch, char *type, char *dir)
{
...
void
do_open (struct char_data *ch, char *argument, int cmd)
{
  int door, other_room;
...
void
do_close (struct char_data *ch, char *argument, int cmd)
{
  int door, other_room;
  char type[MAX_INPUT_LENGTH], dir[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH];
  struct room_direction_data *back;
...
void
do_lock (struct char_data *ch, char *argument, int cmd)
{
  int door, other_room;
...
void
do_unlock (struct char_data *ch, char *argument, int cmd)
{
  int door, other_room;
  char type[MAX_INPUT_LENGTH], dir[MAX_INPUT_LENGTH];
  struct room_direction_data *back;
...
void
do_pick (struct char_data *ch, char *argument, int cmd)
{
  byte percent;
  int door, other_room;
  char type[MAX_INPUT_LENGTH], dir[MAX_INPUT_LENGTH];
...
void
do_enter (struct char_data *ch, char *argument, int cmd)
{
  int door;
...
void
do_leave (struct char_data *ch, char *argument, int cmd)
{
  int door;
...
void
do_stand (struct char_data *ch, char *argument, int cmd)
{
  switch (GET_POS (ch))
    {
...
void
do_sit (struct char_data *ch, char *argument, int cmd)
{
  switch (GET_POS (ch))
    {
    case POSITION_STANDING:
...
void
do_rest (struct char_data *ch, char *argument, int cmd)
{
  switch (GET_POS (ch))
    {
    case POSITION_STANDING:
...
void
do_sleep (struct char_data *ch, char *argument, int cmd)
{
  switch (GET_POS (ch))
    {
    case POSITION_STANDING:
...
void
do_wake (struct char_data *ch, char *argument, int cmd)
{
...
void
do_follow (struct char_data *ch, char *argument, int cmd)
{
  char name[160];
  ...
  if (IS_SET (leader->specials.act, PLR_SOLO))
    {
  ...
  if (IS_AFFECTED (ch, AFF_CHARM) && (ch->master))
      ...
      else
	{
	  if (circle_follow (ch, leader))
	    {
	      act ("Sorry, but following in 'loops' is not allowed", FALSE,
		   ch, 0, 0, TO_CHAR);
	      return;
	    }
	  if (ch->master)
