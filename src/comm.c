...
#define MINUTES(m)	(60*(m))
  ...
  if (tod < MINUTES (1) && hour != th)
	...
	{
	  reboot_time += A_DAY - (boottime + reboot_time - 1000) % A_DAY -
...
void
send_to_char_han (char *msgeng, char *msghan, struct char_data *ch)
{
     ...
     struct obj_data *obj, void *vict_obj, int type)
{
  ...
  for (; to; to = to->next_in_room)
	...
	{
	  for (strp = str, point = buf; (point - buf) <= MAX_STRING_LENGTH -
	       1;)
	    if (*strp == '$')
	      {
		switch (*(++strp))
...
void
acthan (char *streng, char *strhan, int hide_invisible, struct char_data *ch,
	struct obj_data *obj, void *vict_obj, int type)
{
...
void
freaky (struct descriptor_data *d)
{
  char buf[128];
