...
int
super_deathcure (struct char_data *ch, int cmd, char *arg)
{
...
int
deathcure (struct char_data *ch, int cmd, char *arg)
{
  ...
  switch (number (0, 40))
    ...
    case 7:
      ...
      return (1);
    case 8:
      act ("$n massacres you to small fragment with his slash .", 1, ch, 0,
	   0, TO_ROOM);
      act ("$n massacres you to small fragment with his slash .", 1, ch, 0,
	   0, TO_ROOM);
      act ("$n massacres you to small fragment with his slash .", 1, ch, 0,
...
int
perhaps (struct char_data *ch, int cmd, char *arg)
{
...
int
Quest_bombard (struct char_data *ch, int cmd, char *arg)
...
/* if ch->in_room == real_room(3035)  .... */
  ...
  if (vict)
      ...
      if (GET_LEVEL (ch) == 35)
	{
	  ...
	  vict->points.hit -= vict->points.hit / 3 - number (1,
							     vict->points.hit
							     / 8 + GET_LEVEL
							     (vict) / 2);
...
int
mud_message (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
  if (cmd)
    return (0);
...
int
musashi (struct char_data *ch, int cmd, char *arg)
{
  void cast_sunburst (byte level, struct char_data *ch, char *arg, int type,
		      struct char_data *victim, struct obj_data *tar_obj);
  struct char_data *choose_victim (struct char_data *mob, int fightmode,
  ...
  if (ch->specials.fighting)
      ...
      else
	{
      ...
      if (ch->specials.fighting && !affected_by_spell (ch, SPELL_SANCTUARY))
	{
	  act ("$n 주위로 휘황한 광채가  감쌉니다.", TRUE, ch,
	       0, 0, TO_ROOM);
	  af.type = SPELL_SANCTUARY;
	  af.duration = 8;
	  af.modifier = 0;
...
int
super_musashi (struct char_data *ch, int cmd, char *arg)
{
  void cast_sunburst (byte level, struct char_data *ch, char *arg, int type,
		      struct char_data *victim, struct obj_data *tar_obj);
  struct char_data *choose_victim (struct char_data *mob, int fightmode,
	...
	case 1:
	  ...
	  for (vict = ch->specials.fighting; vict; vict = next_vict)
		...
		break;
	      ...
	      if (vict->specials.fighting == ch)
		{
		  cast_corn_of_ice (GET_LEVEL (ch), ch, "",
...
int
mom (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
  int h;
...
int
singer (struct char_data *ch, int cmd, char *arg)
{
  static int n = 0;
  if (cmd)
    return FALSE;
...
int
rescuer (struct char_data *ch, int cmd, char *arg)
{
  ...
  if (vict == ch)
	...
	case 3:
      ...
      if (vict)
	{
	  act ("$n screams 'PROTECT THE INNOCENT! CHARGE!'", FALSE, ch, 0,
	       0, TO_ROOM);
...
int
superguard (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *tch, *evil;
...
int
hospital (struct char_data *ch, int cmd, char *arg)
{
...
int
metahospital (struct char_data *ch, int cmd, char *arg)
{
  char buf[MAX_STRING_LENGTH];
...
int
remortal (struct char_data *ch, int cmd, char *arg)
{
...
int
jale_room (struct char_data *ch, int cmd, char *arg)
{
...
int
safe_house (struct char_data *ch, int cmd, char *arg)
{
  char buf[100];
...
int
shooter (struct char_data *ch, int cmd, char *arg)
{
...
int
finisher (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
  int max_dam = 0;
...
int
totem (struct char_data *ch, int cmd, char *arg)
{
  int rm;
  struct obj_data *obj;
...
int
kickbasher (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
...
int
spell_blocker (struct char_data *ch, int cmd, char *arg)
{
  if (cmd == 84)
    {
...
int
archmage (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
...
int
spitter (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
				...
				CLASS_CLERIC)))
	{
	  switch (number (1, 7))
	    ...
	    case 6:
	      ...
	      cast_blindness (GET_LEVEL (ch), ch, "", SPELL_TYPE_SPELL,
			      vict, 0);
	      return (FALSE);
	    default:
	      return (FALSE);
...
int
portal (struct char_data *ch, int cmd, char *arg)
{
  int location, ok;
...
int
magicseed (struct char_data *ch, int cmd, char *arg)
{
  int skno;
  char name[256];
  struct obj_data *obj;
