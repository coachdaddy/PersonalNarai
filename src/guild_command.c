...
int
taxi (struct char_data *ch, int cmd, char *arg)
{
...
int
guild_entry (struct char_data *ch, int cmd, char *arg)
{
...
int
locker_room (struct char_data *ch, int cmd, char *arg)
{
  char buf[MAX_STRING_LENGTH];
  int i, room_number;
  char guild_number;
...
int
guild_practice_yard (struct char_data *ch, int cmd, char *arg)
{
  char buf[MAX_STRING_LENGTH];
  int i, room_number;
  char guild_number;
  ...
  if (cmd == 275)
	...
	{
	case POLICE:
	  cost = police_skill_costs[number] * ((GET_GUILD_SKILL (ch, number)
						/ 10) + 1) * GET_LEVEL (ch);
	  sprintf (buf, "You train %s.\n\r", police_skills[number]);
	  send_to_char (buf, ch);
	  break;
	case OUTLAW:
	  cost = outlaw_skill_costs[number] * ((GET_GUILD_SKILL (ch, number)
						/ 10) + 1) * GET_LEVEL (ch);
      ...
      if (GET_EXP (ch) > cost)
	{
	  ...
	  GET_GUILD_SKILL (ch, number) = MIN (GET_GUILD_SKILL (ch, number) +
					      10, 90);
	}
      else
	{
