			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	if (GET_ITEM_TYPE(obj) == ITEM_DRINKCON) {
		if ((obj->obj_flags.value[2] != LIQ_WATER)
			...
			water = 2 * level * ((weather_info.sky >= SKY_RAINING)
					     ? 2 : 1);
			/* Calculate water it can contain, or water created */
			water = MIN(obj->obj_flags.value[0] -
			 ...
			 struct char_data *victim, struct obj_data *obj)
{
	...
	if (GET_ITEM_TYPE(obj) == ITEM_DRINKCON) {
		if ((obj->obj_flags.value[2] != LIQ_NECTAR)
			...
			nectar = 2 * level;
			/* Calculate water it can contain, or water created */
			nectar = MIN(obj->obj_flags.value[0] -
		     ...
		     struct char_data *victim, struct obj_data *obj)
{
	...
	if ((GET_ITEM_TYPE(obj) == ITEM_WAND) ||
		...
		if (!number(0, level / 10 + 1)) {
			act("BANG!!!! $p explodes. It hurts!", FALSE, ch, obj,
			    0, TO_CHAR);
			extract_obj(obj);
			  ...
			  struct char_data *victim, struct obj_data *obj)
{
	...
	if (number(0, 9) > 8) {
		...
		act("PoooooooYa! You gain more hit point!", TRUE, victim, 0,
		    0, TO_CHAR);
		GET_PLAYER_MAX_HIT(victim) += number(1, GET_CON(victim)) * number
		    (1, 3);
	} else if (number(0, 9) > 8) {
		act("Shaaaaaaru! $n gain more move point!", TRUE, victim, 0,
		    0, TO_ROOM);
		act("Shaaaaaaru! You gain more move point!", TRUE, victim, 0,
		    0, TO_CHAR);
	...
	if (GET_ITEM_TYPE(obj) == ITEM_WEAPON) {
		...
		if (!IS_SET(obj->obj_flags.extra_flags, ITEM_MAGIC)) {
			...
			if (IS_GOOD(ch)) {
				...
				act("$p glows blue evilly.", FALSE, ch, obj,
				    0, TO_CHAR);
			} else if (IS_EVIL(ch)) {
				SET_BIT(obj->obj_flags.extra_flags, ITEM_ANTI_GOOD);
				act("$p glows red saintly.", FALSE, ch, obj,
				    0, TO_CHAR);
			} else {
		...
		/* if(ITEM_MAGIC) end */
            ...
            if (number(0, 100000) < 5) { /* if (number(0, 1000) < 5 )  */
				...
				act("You can't open your eyes.", FALSE, ch,
				    obj, 0, TO_CHAR);
			...
			struct char_data *victim, struct obj_data *obj)
{
	...
	if (obj) {
		...
		if (!affected_by_spell(victim, SPELL_INVISIBLE)) {
			act("$n slowly fade out of existence.", TRUE, victim,
			    0, 0, TO_ROOM);
		 ...
		 struct char_data *victim, struct obj_data *obj)
{
	...
	if (!saves_spell(victim, SAVING_PARA)) {
		...
		affect_join(victim, &af, FALSE, FALSE);
		if (GET_POS(victim) > POSITION_SLEEPING) {
			act("You feel very sleepy ..... zzzzzz", FALSE,
			    victim, 0, 0, TO_CHAR);
