...
int
guild (struct char_data *ch, int cmd, char *arg)
{
  ...
  if (!*arg)
      ...
      for (i = 0; *spells[i] != '\n'; i++)
	{
	  if (*spells[i] &&
	    ...
	    {
	      sprintf (tmp, "%-20s %-4s\n\r", spells[i], how_good
		       (ch->skills[i + 1].learned, ch->skills[i + 1].skilled));
	      strcat (buf3, tmp);
	    }
	}
  ...
  switch (path[index])
    ...
    case 'S':
      ...
      acthan ("$n lies down and instantly falls asleep.",
	      "$n님이 자리에 눕더니 금새 잠이 듭니다.",
	      FALSE, ch, 0, 0, TO_ROOM);
      break;

    case 'a':
      acthan ("$n says 'Hello Honey!'",
	      "$n님이 '안녕 내사랑!' 이라고 말합니다.", FALSE,
	      ch, 0, 0, TO_ROOM);
      acthan ("$n smirks.", "$n님이 점잔빼며 웃습니다.", FALSE,
	      ch, 0, 0, TO_ROOM);
      break;
...
void
npc_tornado (struct char_data *ch)
{
...
void
first_attack (struct char_data *ch, struct char_data *victim)
{
...
*/
int
warrior (struct char_data *ch, int cmd, char *arg)
{
...
thief (struct char_data *ch, int cmd, char *arg)
{
  ...
  if (GET_POS (ch) == POSITION_FIGHTING)
	...
	{
	case 1:
	  do_backstab (ch, GET_NAME (victim), 0);
	  break;
	case 2:
	  do_light_move (ch, GET_NAME (victim), 0);
	  break;
	case 3:
	  do_flash (ch, GET_NAME (victim), 0);
	  break;
	case 4:
	  npc_tornado (ch);
	case 5:
	  do_punch (ch, GET_NAME (victim), 0);
	  break;
	}
      return TRUE;
    }
...
int
paladin (struct char_data *ch, int cmd, char *arg)
{
  struct char_data *vict;
...
int
dragon (struct char_data *ch, int cmd, char *arg)
{
  int mh;
  struct char_data *vict;
