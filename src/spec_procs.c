...
int warrior(struct char_data *ch, int cmd, char *arg)
{
	...
	if (GET_POS(ch) == POSITION_FIGHTING) {
		...
		if (cmd_info[do_what_cmd[do_what]].minimum_level[3] <
		    GET_LEVEL(ch)) {
...
int paladin(struct char_data *ch, int cmd, char *arg)
{
	...
	case 40:
		...
		if (!IS_AFFECTED(ch, AFF_SPELL_BLOCK))
			cast_full_heal(GET_LEVEL(ch), ch, "",
				       SPELL_TYPE_SPELL, ch, 0);
		else
...
int dragon(struct char_data *ch, int cmd, char *arg)
{
	...
	case 3:
		...
		cast_lightning_breath(GET_LEVEL(ch), ch, "", SPELL_TYPE_SPELL,
				      vict, 0);
		if (mh < 400)
			return TRUE;
	case 4:
