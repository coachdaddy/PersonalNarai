/* ************************************************************************
*  file: mob_magic.c , Implementation of mob spells.	  Part of DIKUMUD *
*  Usage : The actual effect for monnster magic usage                     *
...
void
mob_spell_fire_storm (byte level, struct char_data *ch,
...
void
mob_light_move (struct char_data *ch, char *argument, int cmd)
{
...
void
mob_punch_drop (struct char_data *ch, struct char_data *victim)
{
  int i;
  ...
  if (mode == MODE_HIT_MIN || mode == MODE_MANA_MIN || mode == MODE_MOVE_MIN
      ...
      for (i = 1; i < count; i++)
	{
	  switch (mode)
	    {
	    case MODE_HIT_MIN:
	      tmp = GET_HIT (victims[i]);
	      break;
	    case MODE_MANA_MIN:
	      tmp = GET_MANA (victims[i]);
	      break;
	    case MODE_MOVE_MIN:
	      tmp = GET_MOVE (victims[i]);
	      break;
	    case MODE_AC_MIN:
	      tmp = GET_AC (victims[i]);
	      break;
	    case MODE_HR_MIN:
	      tmp = GET_DAMROLL (victims[i]);
	      break;
	    case MODE_DR_MIN:
	      tmp = GET_HITROLL (victims[i]);
	      break;
	    case MODE_LEVEL_MIN:
	      tmp = GET_LEVEL (victims[i]);
	      break;
	    default:
	      tmp = GET_HIT (victims[i]);
	    }
...
choose_rescue_mob (struct char_data *mob)
{
  ...
  if (count == 0)
    {				/* there is no mob fighting except me.. */
