...
struct mbox_data *
init_a_mbox (struct char_data *ch)
{
...
int
mbox (struct char_data *ch, int cmd, char *arg)
{
  struct mbox_data *cur_mbox;

  if (!ch->desc)
    return (FALSE);
  if (cmd != 15 && cmd != 247 && cmd != 10)
    return (FALSE);
...
int
show_mail (struct char_data *ch, struct mbox_data *cb, char *arg)
{
  int i, pm_num = 0;
...
void
remove_mail (struct mbox_data *cb, int m_num)
{
  int ind;
...
int
get_mail (struct char_data *ch, struct mbox_data *cb, char *arg)
{
...
int
post_mail (struct char_data *ch, struct mbox_data *cb, char *arg)
{
