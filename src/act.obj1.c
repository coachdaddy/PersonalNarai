	...
	if (sub_object) {
		...
		if (sub_object->carried_by == ch) {
			act("You get $p from $P.", 0, ch, obj_object, sub_object,
			    TO_CHAR);
			act("$n gets $p from $s $P.", 1, ch, obj_object,
			    sub_object, TO_ROOM);
		} else {
			act("You get $p from $P.", 0, ch, obj_object, sub_object,
			    TO_CHAR);

			act("$n gets $p from $P.", 1, ch, obj_object,
			    sub_object, TO_ROOM);
		}
	} else {
				...
				if (CAN_SEE_OBJ(ch, obj_object)) {
					if ((IS_CARRYING_N(ch) + 1) <=
						...
						if ((IS_CARRYING_W(ch) +
						    ...
						    CAN_CARRY_W(ch)) {
							if
							    (CAN_WEAR(obj_object, ITEM_TAKE)) {
								get(ch,
								    obj_object, sub_object);
								found = TRUE;
							} else {
		...
		if (*arg2) {
			...
			if (gun) {
				...
				if (ammo) {
					...
					gun->obj_flags.value[0] =
					    gun->obj_flags.value[1];
					if (gun->obj_flags.value[2] >
					    gun->obj_flags.value[1])
