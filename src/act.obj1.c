...
void
get (struct char_data *ch, struct obj_data *obj_object,
...
void
do_get (struct char_data *ch, char *argument, int cmd)
{
...
void
do_drop (struct char_data *ch, char *argument, int cmd)
{
...
void
do_put (struct char_data *ch, char *argument, int cmd)
{
  char buffer[MAX_OUTPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_give (struct char_data *ch, char *argument, int cmd)
{
  ...
  if (is_number (obj_name))
      ...
      if (amount < 0)
	{
      ...
      if ((GET_GOLD (ch) < amount) && (IS_NPC (ch) || (GET_LEVEL (ch) < (IMO
									 + 1))))
	{
	  send_to_char ("You haven't got that many coins!\n\r", ch);
	  return;
...
void
do_reload (struct char_data *ch, char *argument, int cmd)
{
  char buffer[MAX_OUTPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
