	...
	if (d == 1 && (ch != 0) && (victim != 0)) {	/* By Knife */
		...
		case WEAPON_ANY_MAGIC:
			...
			case 4:
				...
				break;
			case 5:
				spell_lightning_bolt(GET_LEVEL(ch), ch,
						     victim, 0);
				break;
			case 6:
				spell_call_lightning(GET_LEVEL(ch), ch,
						     victim, 0);
				break;
			case 7:
				spell_energy_drain(GET_LEVEL(ch), ch, victim, 0);
...
void sweapon_smash(struct char_data *ch, struct char_data *victim, struct obj_data *weapon)
{
	...
	if (GET_CLASS(ch) == CLASS_MAGIC_USER || GET_CLASS(ch) == CLASS_CLERIC)
		dam <<= 2;
	act("$p smashes $n with bone crushing sound.", TRUE, victim, weapon,
	    0, TO_ROOM);
	act("You are smashed by $p. You are hard to stand.", TRUE, victim,
	    weapon, 0, TO_CHAR);
