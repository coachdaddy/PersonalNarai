...
void
new_char (struct char_file_u *ch, struct char_file_u *ch2)
{
  int i;
...
do_find (char *str)
{
  ...
  while (!feof (fp))
    {
      if (fread (&ch, sizeof (struct char_file_u), 1, fp) == 0)
	  continue;
      DEBUG ("Name = %s\n", ch.name);
...
do_show (char *str)
{
  ...
  if (ch.remortal & REMORTAL_MAGIC_USER)
    fprintf (stdout, "M");
  if (ch.remortal & REMORTAL_CLERIC)
    fprintf (stdout, "C");
  if (ch.remortal & REMORTAL_THIEF)
    fprintf (stdout, "T");
  if (ch.remortal & REMORTAL_WARRIOR)
    fprintf (stdout, "W");
  fprintf (stdout, "\n");
