...
PrintAct (int act)
{
  ...
  if (IS_SET (act, ACT_SCAVENGER))
    strcat (buf, "PICKUP ");
  if (IS_SET (act, ACT_NICE_THIEF))
    strcat (buf, "STEAL ");
  if (IS_SET (act, ACT_AGGRESSIVE))
    strcat (buf, "AGGR ");
  if (IS_SET (act, ACT_STAY_ZONE))
    strcat (buf, "STAY ");
  if (IS_SET (act, ACT_WIMPY))
    strcat (buf, "WIMPY ");
  if (IS_SET (act, ACT_PALADIN))
    strcat (buf, "PALADIN ");
  if (IS_SET (act, ACT_DRAGON))
    strcat (buf, "DRAGON ");
  if (IS_SET (act, ACT_SPITTER))
    strcat (buf, "SPIT ");
  if (IS_SET (act, ACT_SHOOTER))
    strcat (buf, "SHOOT ");
  if (IS_SET (act, ACT_GUARD))
    strcat (buf, "GUARD ");
  if (IS_SET (act, ACT_SUPERGUARD))
    strcat (buf, "SUPERGUARD ");
  if (IS_SET (act, ACT_HELPER))
    strcat (buf, "HELPER ");
  if (IS_SET (act, ACT_RESCUER))
    strcat (buf, "RESCUER ");
  if (IS_SET (act, ACT_SPELL_BLOCKER))
    strcat (buf, "BLOCKER ");
  if (IS_SET (act, ACT_FINISH_BLOW))
    strcat (buf, "BLOWER ");
...
void
PrintAffected (int affected)
{
  char buf[256];

  buf[0] = 0;
  if (IS_SET (affected, AFF_BLIND))
    strcat (buf, "BLIND ");
  if (IS_SET (affected, AFF_INVISIBLE))
    strcat (buf, "INVIS ");
  if (IS_SET (affected, AFF_DETECT_EVIL))
    strcat (buf, "D.EVIL ");
  if (IS_SET (affected, AFF_DETECT_INVISIBLE))
    strcat (buf, "D.INV ");
  if (IS_SET (affected, AFF_INFRAVISION))
    strcat (buf, "INFRA ");
  if (IS_SET (affected, AFF_SENSE_LIFE))
    strcat (buf, "SENSE ");
  if (IS_SET (affected, AFF_DEAF))
    strcat (buf, "DEAF ");
  if (IS_SET (affected, AFF_SANCTUARY))
    strcat (buf, "SANC ");
  /*
     if(IS_SET(affected, AFF_GROUP)) strcat(buf, "GROUP ");
   */
  if (IS_SET (affected, AFF_DUMB))
    strcat (buf, "DUMB ");
  if (IS_SET (affected, AFF_CURSE))
    strcat (buf, "CURSE ");
  if (IS_SET (affected, AFF_MIRROR_IMAGE))
    strcat (buf, "M.IMAGE ");
  if (IS_SET (affected, AFF_POISON))
    strcat (buf, "POISON ");
  if (IS_SET (affected, AFF_PROTECT_EVIL))
    strcat (buf, "P.EVIL ");
  if (IS_SET (affected, AFF_REFLECT_DAMAGE))
    strcat (buf, "R.DAM ");
  /*
     if(IS_SET(affected, AFF_HOLY_SHIELD)) strcat(buf, "H.SLD ");
   */
  if (IS_SET (affected, AFF_SPELL_BLOCK))
    strcat (buf, "S.BLOCK ");
  if (IS_SET (affected, AFF_SLEEP))
    strcat (buf, "SLEEP ");
  if (IS_SET (affected, AFF_SHADOW_FIGURE))
    strcat (buf, "SHADOW ");
  if (IS_SET (affected, AFF_SNEAK))
    strcat (buf, "SNEAK ");
  if (IS_SET (affected, AFF_HIDE))
    strcat (buf, "HIDE ");
  if (IS_SET (affected, AFF_DEATH))
    strcat (buf, "DEATH ");
  /* 
     if(IS_SET(affected, AFF_CHARM)) strcat(buf, "CHARM ");
     if(IS_SET(affected, AFF_FOLLOW)) strcat(buf, "FOLLOW ");
   */
  if (IS_SET (affected, AFF_HASTE))
    strcat (buf, "HASTE ");
  if (IS_SET (affected, AFF_IMPROVED_HASTE))
    strcat (buf, "I.HASTE ");
  if (IS_SET (affected, AFF_LOVE))
    strcat (buf, "LOVE ");
  if (IS_SET (affected, AFF_RERAISE))
    strcat (buf, "RERAISE ");
  if (IS_SET (affected, AFF_ARREST))
    strcat (buf, "ARREST ");
