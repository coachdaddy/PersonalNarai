...
int main()
{
	...
	fprintf(mobfile, "%ld %ld %d\n", mob_data.special_act,
		mob_data.affected_by, mob_data.alignment);

	fprintf(mobfile, "%c %d %d %d %d %d\n", mob_data.class,
		mob_data.level, mob_data.hitpoint,
		mob_data.armor, mob_data.hr, mob_data.dr);
	fprintf(mobfile, "%lld %lld %d %c\n", mob_data.gold, mob_data.xp,
		mob_data.position, mob_data.sex);
...
int ask_question()
{
	...
	for (;;) {
		scanf("%c", &answer);
		getc(stdin);
		if (answer == 'Y' || answer == 'N' || answer == 'y' || answer
		    == 'n')
			break;
		else
